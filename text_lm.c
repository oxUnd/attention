...
static void generate_text_batch_at(const int *corpus_idx, Tensor3D *src, Tensor3D *tgt, int *targets,
...
void text_lm_generate(Transformer *model, const TextLmHyperparams *hp, const char *seed, int length,
    ...
    for (int step = 0; step < length; step++) {
        ...
        Tensor3D src = tensor_create(1, cur_seq_len, d_model);
        Tensor3D tgt = tensor_create(1, cur_seq_len, d_model);

        for (int s = 0; s < cur_seq_len; s++) {
            int ci = buf[start + s];
            for (int d = 0; d < d_model; d++) {
        ...
        int pred = sample_output(&output.data[(cur_seq_len - 1) * d_model], TEXT_LM_VOCAB_SIZE, temperature);
        buf[buf_len++] = pred;
...
int text_lm_train(const TextCorpus *corpus, const TextLmHyperparams *hp, Transformer **model_out,
    ...
    for (int epoch = 0; epoch < hp->max_epochs; epoch++) {
        ...
        for (int bi = 0; bi < num_batches; bi++) {
            ...
            generate_text_batch_at(corpus_idx, &src, &tgt, targets, hp->d_model, seq_len, batch_size, starts);
            ...
            LossResult loss = cross_entropy_loss(&output, targets, TEXT_LM_VOCAB_SIZE);
            float acc = compute_accuracy(&output, targets, seq_len, batch_size, TEXT_LM_VOCAB_SIZE);
        ...
        if (epoch % 5 == 0 || epoch == hp->max_epochs - 1 || avg_loss < 1.2f) {
        ...
        if (patience >= hp->early_stop_patience || avg_loss < 0.35f) {
